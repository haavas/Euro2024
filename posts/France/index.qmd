...
The less things change, the more they stay the same. France continued their underwhelming progress tonight. Mbappé did not score, and we got another own goal to our tally. 
...
This game changed nothing standings as far as I can see. All of us had France in the next round, but not necessarily in this particular quarter-final. 

```{r knock_41, echo = FALSE, warning=FALSE,  fig.height=10, fig.width=10}

invisible(Sys.setlocale())
poeng_prev <- tbl_df(read.csv("details40.csv", sep=";", header = FALSE, encoding = "utf-8"))
poeng_prev$total_prev <- poeng_prev$V2 + poeng_prev$V3 + poeng_prev$V4
poeng_prev <- poeng_prev %>% select(V1, total_prev)

poeng <- tbl_df(read.csv("details41.csv", sep=";", header = FALSE))
